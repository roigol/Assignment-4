...
public class BTree<T extends Comparable<T>> {
    ...
    private int minChildrenSize = 2; // 2
    private int maxKeySize = 3; // 2
    private int maxChildrenSize = 4; // 3
    ...
    public boolean insert(T value) {
        if (root == null) {
            ...
            if (root.numberOfKeys() == maxKeySize)
                split(root);
            Node<T> node = root;
            while (node != null) {
                ...
                // Lesser or equal
                T lesser = node.getKey(0);//firs index key
                if (value.compareTo(lesser) <= 0) {//if value is "smaller" then it
                    Node<T> leftC = node.getChild(0);//leftmost child
                    T mid = leftC.getKey(leftC.numberOfKeys() / 2);//median index key
                    if (leftC.numberOfKeys() == maxKeySize) {//split is needed
                        split(leftC);
                        splitted = true;
                    }
                    if (splitted & value.compareTo(mid) > 0)
                ...
                if (value.compareTo(greater) > 0) {
                    Node<T> rightC = node.getChild(numberOfKeys);
                    T mid = rightC.getKey(rightC.numberOfKeys() / 2);
                    if (rightC.numberOfKeys() == maxKeySize) {
                        split(rightC);
                        splitted = true;
                    }
                    if (splitted & value.compareTo(mid) > 0)
                        node = node.getChild(numberOfKeys + 1);
                    else node = node.getChild(numberOfKeys);
                    continue;
                }

                // Search internal nodes - **go somewhere in the middle**
                for (int i = 1; i < node.numberOfKeys(); i++) {
                    T prev = node.getKey(i - 1);
                    T next = node.getKey(i);
                    if (value.compareTo(prev) > 0 && value.compareTo(next) <= 0) {
                        Node<T> curr = node.getChild(i);
                        T mid = curr.getKey(curr.numberOfKeys() / 2);
    ...
    public T delete(T value) {
        ...
        if (root != null) {
            ...
            while (!removed & node != null) {
                if (node.numberOfKeys() <= minKeySize & node.parent != null)
                    combined(node);
    ...
    public boolean insert2pass(T value) {
        ...
        if (root == null) {
            ...
            Node<T> node = root;
            Node<T> father = null;
            boolean fatherDefined = false;
            while (node != null) {
                if (!fatherDefined & node.numberOfKeys() == maxKeySize) {
                    father = node;
                    fatherDefined = true;
                }
                ...
                if (node.numberOfChildren() == 0) {
                    if (node.numberOfKeys() < maxKeySize) {
                        ...
                        if (root == father) {
                            ...
                            BTree<T> tree = new BTree<T>(order);
                            tree.root = father.parent;
                            tree.insert(value);
                            size++;
                        }
                        break;
                ...
                // Lesser or equal - **we need to go left**
                T lesser = node.getKey(0);
                if (value.compareTo(lesser) <= 0) {
                    node = node.getChild(0);
                    continue;
    ...
    private T remove(T value, Node<T> node) {
        ...
        if (node.numberOfChildren() == 0) {
            // leaf node
            removed = node.removeKey(value);
            if (node.parent != null && node.numberOfKeys() < minKeySize) {//if node is not the root and has less then min
                this.combined(node);
            } else if (node.parent == null && node.numberOfKeys() == 0) {//if node is the root and its empty
            ...
            //replacing with value's predecessor
            Node<T> lesser = node.getChild(index);//value's left child
            if (lesser.numberOfKeys() > minKeySize) {
                removed = node.removeKey(value);
                Node<T> greatest = this.getGreatestNode(lesser);//returns the rightmost child of lesser
                T replaceValue = this.removeGreatestValue(greatest);//returns and delete the rightmost key of greatest (value's successor)
                node.addKey(replaceValue);//adding the successor
    ...
    private Node<T> grtSmallestNode(Node<T> nodeToGet) {
        Node<T> node = nodeToGet;
        while (node.numberOfChildren() > 0) {
            node = node.getChild(0);
        }
    ...
    private boolean combined2(Node<T> node, boolean merge) {
        ...
        // Try to borrow a key from a neighbor
        ...
        if (leftNeighbor != null && leftNeighborSize > minKeySize & !merge) {
            ...
            if (rightNeighbor != null && rightNeighborSize > minKeySize & !merge) {
                ...
                node.addKey(parentValue);
                parent.addKey(neighborValue);
                if (rightNeighbor.numberOfChildren() > 0) {//**relocate the R neighbors leftmost child**
                ...
                // Can't borrow from neighbors, try to merge with left neighbor..................................
                ...
                // Can't borrow from neighbors, try to merge with right neighbor.................................
                T removeValue = rightNeighbor.getKey(0);
                int prev = getIndexOfPreviousValue(parent, removeValue);
                T parentValue = parent.removeKey(prev);
